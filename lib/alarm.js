...
function listv2(callback){
  list(function(alarms){
    var funcs = alarms.map(function(e){
      return function(cb){
        fs.readFile('./files/' + e + '.sh', 'utf8', function(err, text){
          cb(null, {
            name: e,
            time: require('../files/' + e + '.json').cron,
            script: text
          });
        });
      }
...
function start(){
  list(function(alarms){
    crons = alarms.map(function(alarm){
      var cron = require('../files/' + alarm + '.json');
      console.log(cron);
      return new CronJob(cron.cron, function() {
        exec('bash ./files/' + alarm + '.sh', function(err, stdout, stderr){
          console.log(stdout);
          console.log(stderr);
        });
